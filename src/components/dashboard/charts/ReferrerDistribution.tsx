    ...
    return (
        <Card className="shadow-sm hover:shadow-md transition-all">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                ...
                <Select value={sortBy}
                        onValueChange={(value: 'revenue' | 'visits' | 'conversion_rate') => setSortBy(value)}>
                                ...
                                <TableBody>
                                    {data.data.map((referrer: ReferrerData) => (
                                        <TableRow key={referrer.source}>
                                            ...
                                            <TableCell
                                                className="text-right">{formatValue(referrer.total_visits, 'number')}</TableCell>
                                            <TableCell
                                                className="text-right">{formatValue(referrer.conversion_rate, 'percentage')}</TableCell>
                                            <TableCell
                                                className="text-right">{formatValue(referrer.total_revenue, 'currency')}</TableCell>
                                            <TableCell className="text-right">
                        <span
                            className={`flex items-center justify-end gap-1 ${referrer.change_percentage > 0 ? 'text-green-600' : referrer.change_percentage < 0 ? 'text-red-600' : 'text-muted-foreground'}`}>
                          {referrer.change_percentage > 0 ?
                              <ArrowUp className="h-4 w-4"/> : referrer.change_percentage < 0 ?
